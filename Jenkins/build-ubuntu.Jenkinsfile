pipeline {
    ...
    parameters {
        string(
            name: 'TAG',
            defaultValue: 'adreno-ubuntu-noble-updates',
            ...
            steps {
                sh '''#!/bin/bash
                    ...
                    apt install -y git ccache build-essential devscripts fakeroot quilt \
                        git-buildpackage pristine-tar meson-1.5 libxfixes-dev libarchive-dev \
                        librust-paste-dev librust-syn-dev bindgen
                '''
            }
        }
...
MDEOF
                    ...
                    sh """
                        ...
                        tag="${params.TAG}"
                        if [[ "\$tag" == *"turnip"* ]]; then
                            title="turnip-\${version}_ubuntu_noble"
                        else
                            title="\${version}_ubuntu_noble"
                        fi
