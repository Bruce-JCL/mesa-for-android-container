                    ...
                    steps {
                        script {
                            ...
                            sh '''#!/bin/bash
                                ...
                                tar -zcvf "turnip_${MESA_RELEASE_NAME_SUFFIX}.tar.gz" \
                                ...
                                cp turnip_${MESA_RELEASE_NAME_SUFFIX}.tar.gz "$WORKSPACE/"
                    ...
                    steps {
                        script {
                            ...
                            sh '''#!/bin/bash
                                ...
                                tar -zcvf "turnip_${MESA_RELEASE_NAME_SUFFIX}.tar.gz" \
                                    -C /tmp/mesa-install-tmp .
                                chmod 644 "turnip_${MESA_RELEASE_NAME_SUFFIX}.tar.gz"
                                rm -rf /tmp/mesa-install-tmp
                                echo "Calculating sha256 checksums..."
                                find . -maxdepth 1 -name "*.tar.gz" -type f \
                                    -printf "%P\\0" | xargs -0 sha256sum | sort -k2 > sha256sums.txt
                        ...
                        script {
                            ...
                            sh """
                                ...
                                sudo -u runner bash -c '
                                    ...
                                    git config --global user.name "Jenkins"
                                    tag="${params.TAG}"
                                    if [ "\$tag" = "turnip-main" ]; then
                                        git clone -b turnip --depth 1 ${PKGBUILD_REPO_URL}
                                    else
                            ...
                            sh '''#!/bin/bash
                                set -euo pipefail
                                cd /home/runner/archlinuxarm-PKGBUILDs/extra/mesa
                                for f in vulkan-freedreno*.pkg.tar.xz; do
                                    [[ -f "$f" ]] || continue
                                    newname="${f//:/-}"
                                    [[ "$f" != "$newname" ]] && mv -v "$f" "$newname"
                                done
                                echo "Calculating sha256 checksums..."
                                find . -maxdepth 1 -name "vulkan-freedreno*.pkg.tar.xz" -type f \
                                    -printf "%P\\0" | xargs -0 sha256sum | sort -k2 > sha256sums.txt
                                cat sha256sums.txt && echo
                                cp sha256sums.txt "$WORKSPACE/sha256sums_archlinux.txt"
                                cp vulkan-freedreno*.pkg.tar.xz "$WORKSPACE/"
                            '''
                        ...
                        script {
                            ...
                            sh '''#!/bin/sh
                                ...
                                xbps-install -y bash base-devel git ccache meson ninja pkg-config python3 \
                                    flex bison \
                                    libdrm-devel \
                                    libX11-devel libxcb-devel libxshmfence-devel \
